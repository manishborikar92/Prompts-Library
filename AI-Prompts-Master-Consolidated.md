...
**Master Edition - All Prompts Consolidated**
...
16. [Web Development & Frontend Frameworks](#16-web-development--frontend-frameworks)
17. [UI/UX Design & Layout Optimization](#17-uiux-design--layout-optimization)
18. [Notification & Messaging Systems](#18-notification--messaging-systems)
19. [Documentation Organization & Cleanup](#19-documentation-organization--cleanup)
20. [Feature Removal & Code Cleanup](#20-feature-removal--code-cleanup)
...
## 16. Web Development & Frontend Frameworks

### 16.1 Next.js Application Restructuring

```
Analyze the Next.js application. The application is working correctly; however, the **code organization and structure need improvement**.

The current implementation does not follow a clean separation of **pages**, **layouts**, and **components**.

Refactor the codebase to:

* Create a proper **folder structure** following Next.js best practices
* Separate **pages**, **layouts**, and **reusable components**
* Introduce shared components such as a **Navbar**, **Footer**, and common UI elements
* Implement proper **route organization**
* Add **layout components** for consistent page structure
* Organize **API routes** logically
* Structure **utility functions** and **helpers** properly
* Improve overall **code readability, maintainability, and scalability**

Update the complete project structure to follow **best practices** for modern Next.js applications.
```

---

## 17. UI/UX Design & Layout Optimization

### 17.1 UI Layout Redesign

```
Recreate the UI correctly and cleanly, following best UI design practices.

**Important**: Do not change the theme, text, fonts, or colors—only redesign the layout to reflect how it should ideally be structured.
...
## 18. Notification & Messaging Systems

### 18.1 Toast Notification Standardization

```
Review the client/ and server/ codebases to fully understand the toast notification implementation and overall project context.

Identify all usages of toast notifications within the application, including their types (e.g., info, warning, error, success) and the messages being passed and displayed.

Update all toast messages to ensure they are:
- **Single-line** (no line breaks or multi-line messages)
- **Limited to 5–7 words** for clarity and readability
- **Concise and actionable** where appropriate
- **Consistent in tone** across the application

Apply the necessary changes in the relevant source files in both the client/ and server/ directories where these messages originate.

Provide:
1. A list of all current toast messages with their locations
2. Updated versions of each message
3. The complete refactored code with all changes applied
```

---

## 19. Documentation Organization & Cleanup

### 19.1 Documentation Audit and Cleanup

```
Review all Markdown (.md) files in the docs/ directory related to [TOPIC].

There are many files with duplicated content, and some should likely be removed or consolidated.

Perform the following tasks:

1. **Identify Important Files**:
   - Files that provide complete knowledge
   - Files with full understanding of the topic
   - Files with comprehensive implementation details
   - Files with unique, valuable information

2. **Identify Unnecessary Files**:
   - Files with duplicated content
   - Files with partial or incomplete information
   - Files that are outdated or superseded
...
4. **Recommendations**:
   - Provide a categorized list indicating files to **keep**
   - Provide a categorized list indicating files to **delete**
   - Suggest consolidation opportunities
   - Recommend renaming for clarity

5. **Consolidation Plan** (if applicable):
   - Outline which files should be merged
   - Suggest structure for consolidated documentation
   - Note any content that needs updating
```

### 19.2 Markdown Documentation Enhancement and Cleanup

```
Rewrite the provided Markdown file into a clean, well-organized, and professional structure while preserving all original content and meaning. Apply the following improvements:

1. **Naming Conventions**: Apply consistent and intuitive naming conventions for headings, files, sections, and code blocks

2. **Heading Hierarchy**: Normalize heading hierarchy (H1–H6) logically to ensure proper document structure

3. **Readability**: Improve readability through clear section grouping, appropriate spacing, and logical ordering

4. **Formatting**: Standardize formatting for:
   - Lists (ordered and unordered)
   - Tables (alignment, headers, spacing)
   - Links (descriptive text, valid URLs)
   - Code fences (language specification, consistent style)
   - Inline code (appropriate use of backticks)

5. **Content Quality**: 
   - Remove redundancy and duplicate content
   - Fix unclear or inconsistent terminology
   - Ensure technical accuracy

6. **Best Practices**: Ensure the final Markdown follows best practices for:
   - Clarity and accessibility
   - Maintainability
   - Documentation standards
   - Version control friendliness

**Important**: Do not add new information or omit any existing details. The goal is to reorganize and clarify existing content.
```

---

## 20. Feature Removal & Code Cleanup
